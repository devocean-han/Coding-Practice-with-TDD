...
function findRedundantConnection5(edges: number[][]): number[] {
	...
	// 변경 가능한 부분: 이웃 노드를 저장하기 위해 Map과 Set을 대신 사용하기: 
	// const neighborNodes: Map<number, Set<number>> = new Map();
	for (let [a, b] of edges) {
	...
	//^ => 실험 결과, Array 대신 Set을 사용하는 것이 약간 더 빠르고, Map과 일반 Obj은 차이가 없다. '인덱스 지정 접근'을 하는 코드가 없는데도 이런 결과라니 흥미롭다. 
	//^ => 공간적으로도 Array 대신 Set을 사용하는 것이 약간 더 절약된다. 
