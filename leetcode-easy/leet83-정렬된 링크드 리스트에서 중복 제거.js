...
function solution(head) {
	// 0. 링크드 리스트 길이가 0일수도 있으므로: 노드가 없거나 1개뿐이라면 그대로 반환해준다. 
	...
	while (pointer.next) {
		...
		// 2-2. 중복이 아니면 그 노드의 값을 기억한다. 그리고 pointer를 전진시킨다. 
		} else {
			pointer = pointer.next;
			currentVal = pointer.val;
		}
		// 2-3. 그 다음 노드들을 확인하면서 중복이면 제거하고 아니면 그 때 값을 다시 기억한다... 이를 끝까지 반복하기.
	}
